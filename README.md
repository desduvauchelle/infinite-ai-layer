...
The packages are not published to npm or crates.io yet. For now, install from a local checkout as described below. Once releases are published, the intended package names are `@infinite-ai/core`, `@infinite-ai/providers`, `infinite-ai-core`, `infinite-ai-providers`, and `infinite-ai-agents`.
...
The SDK does not install or start Ollama, LM Studio, llama.cpp, Codex CLI, or any cloud provider. Install, run, and authenticate the providers you choose before registering them with the SDK.
...
`pnpm build:sdk` builds the TypeScript core and provider packages. `cargo build --workspace` builds the Rust core, providers, conformance package, and terminal-agent package.
...
```bash
pnpm add @infinite-ai/core @infinite-ai/providers
```
...
Add the terminal-agent package only when the application needs Codex CLI execution:
...
- A permissioned Codex CLI adapter in Rust.
...
See [the SDK guide](./docs/quickstart.md) for streaming, structured output, tools, embeddings, transcription, boundary policy, and Codex agent examples. See [compatibility](./docs/compatibility.md) for the exact tested matrix.
...
Open `http://127.0.0.1:5173`. Use **Connections** to register an already-running local or cloud provider, test it, choose one of its models, and return to the chat. Connection credentials live only in the local Node demo process and are never returned to browser code. Restarting the demo server clears registered connections. Chat history is demo-only browser state, not SDK state.
...
- Apple Intelligence, speech generation, image generation/editing, video generation/editing, Claude CLI, and Gemini CLI are planned follow-on adapters.
